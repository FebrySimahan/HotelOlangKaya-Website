...
<body>
    ...
    <main>
        <div class="pemisah1">
            <h2 style="text-align: center; color: #824D69; margin-right: 59%;"><strong>Room Booking</strong>
            </h2>
            ...
            <div class="container booking-container">
                <div class="row">
                    <div class="col-md-6">
                        <div class="card">
                            <div class="card-body">
                                ...
                                <br>
                                <div class="container">
                                    <h3>Booking Room: {{ $roomTypes->name_types }}</h3>
                                    <br>
                                    <form id="cancelForm" action="{{ route('transaksi_kamar.store') }}"
                                        method="POST" enctype="multipart/form-data">
                                        @csrf
                                        <input type="hidden" name="id_booking" value="{{ $booking->id_booking }}">
                                        <input type="hidden" name="id_room_type"
                                            value="{{ $roomTypes->id_room_type }}">

                                        <div class="mb-1">
                                            <input type="hidden" name="extra_bed" value="0">
                                            <label for="extra_bed" class="form-label">Extra bed (+IDR 250,000)</label>
                                            <input type="checkbox" class="form-check-input" id="extra_bed" name="extra_bed[]" value="1" style="margin-left:10px">
                                            @error('extra_bed')
                                        ...
                                        {{-- <div class="mb-1">
                                            <label for="extra_bed" class="form-label">Extra bed (+IDR 250,000)</label>
                                            <input type="checkbox" class="form-check-input" id="extra_bed"
                                                name="extra_bed" value="1" style="margin-left:10px">
                                            @error('extra_bed')
                                                <div class="invalid-feedback">{{ $message }}</div>
                                            @enderror
                                        </div> --}}
                                        ...
                                        {{-- <div class="mb-2">
                                            <label for="jumlah_kamar"
                                                class="form-label @error('jumlah_kamar') is-invalid @enderror">Number
                                                of Rooms</label>
                                            <div class="input-group" style="width:140px">
                                                <button type="button" class="btn btn-outline-secondary"
                                                    id="decrease" onclick="changeQuantity(-1)">-</button>
                                                <input type="number" class="form-control" id="jumlah_kamar"
                                                    name="jumlah_kamar" value="1" min="1" required>
                                                <button type="button" class="btn btn-outline-secondary"
                                                    id="increase" onclick="changeQuantity(1)">+</button>
                                            </div>
                                            @error('jumlah_kamar')
                                                <div class="invalid-feedback">{{ $message }}</div>
                                            @enderror
                    ...
                    <div class="col-md-6">
                        <div class="card">
                            <div class="card-body">
                                <h5 class="card-title text-center">Kamar Booking</h5>
                                <div class="text-center mb-3">
                                    <img src="{{ asset('AsetGambarRoom/' . $roomTypes->room_picture) }}"
                                        alt="Room image" class="img-fluid" style="border-radius: 8px;width:40%">
                                    <br><br>
                                    <p style="margin-bottom: 0;"><strong>{{ $roomTypes->name_type }}</strong></p>
                                ...
                                <div class="d-flex justify-content-between">
                                    ...
                                    <p style="text-align: right; max-width: 500px">{{ $roomTypes->facility }}</p>
                                </div>

                                <div class="d-flex justify-content-between">
                                    <p><strong>Capacity</strong></p>
                                    <p>{{ $roomTypes->maximum_people }} Person</p>
                                </div>
                                <div class="d-flex justify-content-between">
                                ...
                                <div class="d-flex justify-content-between">
                                    <p>{{ $roomTypes->name_type }}</p>
                                    <p><strong>IDR {{ number_format($roomTypes->price, 0, ',', '.') }}</strong></p>
                                </div>
            ...
            navbarLinks.forEach(link => {
                link.addEventListener('click', function(event) {
                    if (event.target.href && !event.target.href.includes('choose_booking') && !event
                        .target.href.includes('transaksi_kamar')) {
    ...
    {{-- Script untuk validasi form jika pilih batal booking --}}
    <script>
    ...
    <script>
        function changeQuantity(amount) {
            let quantityInput = document.getElementById('jumlah_kamar');
            let currentValue = parseInt(quantityInput.value);

            if (!isNaN(currentValue) && currentValue + amount >= 1) {
